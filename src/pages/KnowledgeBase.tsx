...
import type {
  ...
  TopicQuestion, TopicResource, TopicRevision, Difficulty, ResourceType, EntityType,
} from '../types';
...
function NotesTab({ node, onUpdate }: { node: SyllabusNode; onUpdate: () => void }) {
  const data = node.data as Topic | Subtopic;
  const [content, setContent] = useState(data.notes_content || '');
  const saved = useRef(data.notes_content || '');
  const timer = useRef<ReturnType<typeof setTimeout>>();

  useEffect(() => { setContent(data.notes_content || ''); saved.current = data.notes_content || ''; }, [data.id, data.notes_content]);
  ...
  const handleChange = (val: string) => {
    ...
    timer.current = setTimeout(async () => {
      if (val !== saved.current) {
        const fn = node.level === 'topic' ? updateTopic : updateSubtopic;
        await fn(node.data.id, { notes_content: val });
        saved.current = val;
        if (node.level === 'topic') detectAndSetStatus(node.data.id);
        onUpdate();
      }
  ...
  return (
    <div>
      <label className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1.5 block">Notes (Markdown supported)</label>
      <textarea value={content} onChange={e => handleChange(e.target.value)} placeholder={"Write your notes here...\n\nSupports **bold**, *italic*, # headings, - lists, `code`."} className="w-full h-80 bg-gray-50 border border-gray-200 rounded-lg p-3 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-gray-900/10 resize-none leading-relaxed" />
      <p className="text-[11px] text-gray-400 mt-2">Auto-saves as you type. Adding notes changes status to &quot;Learning&quot;.</p>
    </div>
  );
}
...
function QuestionsTab({ node, onUpdate }: { node: SyllabusNode; onUpdate: () => void }) {
  ...
  const handleAdd = async () => {
    await createTopicQuestion(node.data.id, 'New Question', '', 'Medium', questions.length);
    loadQuestions();
    if (node.level === 'topic') detectAndSetStatus(node.data.id);
    onUpdate();
  };

  const handleUpdate = async (id: string, updates: Partial<TopicQuestion>) => {
  ...
  return (
    <div>
      ...
      {questions.length === 0 ? (
        <div className="text-center py-8"><HelpCircle size={24} className="mx-auto text-gray-300 mb-2" /><p className="text-sm text-gray-400">No questions yet</p><p className="text-xs text-gray-400">Adding questions changes status to &quot;Practicing&quot;</p></div>
      ) : (
...
function QuestionCard({ question, onUpdate, onDelete }: { question: TopicQuestion; onUpdate: (id: string, u: Partial<TopicQuestion>) => void; onDelete: (id: string) => void }) {
  const [editing, setEditing] = useState(false);
...
function ResourcesTab({ node }: { node: SyllabusNode }) {
  ...
  const handleAdd = async () => {
    ...
    await createTopicResource(node.data.id, newTitle, newUrl, newType, resources.length);
    setNewTitle(''); setNewUrl(''); setNewType('Website'); setAdding(false);
    loadResources();
  };
  ...
  return (
    <div>
      ...
      {resources.length === 0 && !adding ? (
        <div className="text-center py-8"><Link size={24} className="mx-auto text-gray-300 mb-2" /><p className="text-sm text-gray-400">No resources yet</p><p className="text-xs text-gray-400">Add links, PDFs, videos, repos</p></div>
      ) : (
        <div className="space-y-2">
          {resources.map(r => (
            <div key={r.id} className="group flex items-center gap-2 bg-white border border-gray-200 rounded-lg px-3 py-2 hover:border-gray-300 transition-colors">
              <span className="text-gray-500 shrink-0">{RESOURCE_ICONS[r.resource_type]}</span>
              <div className="min-w-0 flex-1">
  ...
  return (
    <div>
      ...
      {revisions.length === 0 && !adding ? (
        <div className="text-center py-8"><RotateCcw size={24} className="mx-auto text-gray-300 mb-2" /><p className="text-sm text-gray-400">No revisions yet</p><p className="text-xs text-gray-400">3+ revisions changes status to &quot;Mastered&quot;</p></div>
      ) : (
        <div className="space-y-2">
          {revisions.map(r => (
